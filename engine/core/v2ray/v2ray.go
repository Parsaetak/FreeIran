...
func (b *Backend) buildDocument(cfg config.Config, opts core.RuntimeOptions) (core.RuntimeConfig, error) {
	...
	cacheKey := core.GenCacheKey(cfg.Fingerprint(), "v2ray", opts.LocalHost, opts.LocalPort, opts.HTTPPort)
	generation := core.GenerationFor("v2ray", opts.BinaryPath, opts.BackendVersion)
