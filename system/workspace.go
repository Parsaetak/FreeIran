...
//
// This is the single portable-layout detector in the package. Since
// v0.9.2 the executable directory is ALWAYS the workspace root, so
// the result no longer influences path resolution; it only answers
// "was this a shipped portable deployment?" for diagnostics and the
// developer settings surface (PortableMode). The former per-platform
// copies in paths_unix.go / paths_windows.go / portable.go were
// obsolete duplicates of this logic and were removed in v0.9.5.
...
// portable.marker file sits next to the running executable. Since
// v0.9.2 every deployment is workspace-rooted, so this helper only
// labels the deployment style for diagnostics and the developer
// settings surface; it never influences path resolution.
//
// The check is read-only and safe to call at any time. Diagnostic
// counterpart of InstalledMode.
